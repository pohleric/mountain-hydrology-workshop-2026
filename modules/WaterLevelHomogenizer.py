...
class WaterLevelHomogenizer:
    ...
    def _normalize_station_params(self, stations_dict):
        ...
        Additionally:
        - Interpret all base_level_dates from YAML
          and  converts them to UTC ISO strings.
        """
        norm = {}
    ...
    @staticmethod
    def select_files(file_list, year, file_type, search_string=None):
        # Optional pre-filter by search_string in filename
        if search_string:
            file_list = [f for f in file_list if search_string.lower() in f.lower()]
        ...
        for lvl in levels:
            for f in candidates:
                if lvl is None:
                    ...
                    if WaterLevelHomogenizer.extract_level(f) == lvl:
                        return f
        return None
    ...
    def detect_file_type(self, filepath):
        ...
        if "Plot Title" in lines[0] and "GMT" in lines[1]:
            ...
            return "vanEssen"
        elif "SDSS" in lines[0] in "".join(lines):
            return "sdss"
        else:
    ...
    def _read_hobo(self, filepath):
        ...
        unit_match2 = re.search(r'Full Range, *([a-zA-Z0-9\/]+)', header2)
        m = re.search(r'(?<=Full Range,)\s*(\S+)', header2)
        header2 = "Full Range, μS/cm (LGR S/N: 20847379, SEN S/N: 20847379)"
        if unit_match:
    ...
    def adjust_base_level(self, df, station_key, start_year):
        """
        Cross-year base level calibration:

        - Use all base levels whose timestamps fall within the time span of df
          (plus a small window).
        - Sort them by timestamp.
        - For each base level, compute an offset = (base_level - wl_corr_at_that_time).
        - Apply that offset to the time interval from this BL timestamp up to
          the next BL timestamp (or end of df), regardless of calendar year.
        - Years without their own BL inherit the offset from the previous BL.

        df['datetime'] must be tz-aware UTC.
        Base level dates are stored as UTC ISO strings in self.stations.
        """
        ...
        # 1) Collect all BLs that fall within [tmin-window, tmax+window]
        all_levels = []
        for year, levels_list in base_levels_all.items():
            ...
            for lvl, dt_str in zip(levels_list, dates_list):
                ...
                if (dt >= tmin - window) and (dt <= tmax + window):
                    all_levels.append({
                        'year': int(year),
                        'level': float(lvl),
                        'date': dt,
        ...
        # If no usable BLs -> wl_final = wl_corr
        if not all_levels:
            df['wl_final'] = df['wl_corr']
            # still fill empty metadata
            years_in_df = sorted(df['datetime'].dt.year.unique())
            for y in years_in_df:
                self.base_level_metadata[(station_key, y)] = {
                    'levels': [],
                    'dates': [],
                    'uncertainties': [],
                    'offset': None,
        ...
        # Sort BLs by time
        all_levels = sorted(all_levels, key=lambda x: x['date'])

        # Initialize wl_final as wl_corr
        df['wl_final'] = df['wl_corr']
        df_idx = df.set_index('datetime')

        used_years = {}

        for i, bl in enumerate(all_levels):
            dt = bl['date']
            lvl = bl['level']
            year_bl = bl['year']

            # nearest wl_corr at BL timestamp
            pos = df_idx.index.get_indexer([dt], method='nearest')[0]
            meas = df_idx['wl_corr'].iloc[pos]
            offset = float(lvl - meas)

            # time slice for this BL: from its time to next BL (or full end)
            start = dt if i > 0 else tmin  # optionally extend first BL back to tmin
            end = all_levels[i + 1]['date'] if (i + 1) < len(all_levels) else (tmax + pd.Timedelta('1s'))

            mask = (df['datetime'] >= start) & (df['datetime'] < end)
            df.loc[mask, 'wl_final'] = df.loc[mask, 'wl_corr'] + offset

            # store per-calendar-year metadata (may aggregate multiple BLs per year)
            meta = used_years.setdefault(year_bl, {
                'levels': [],
                'dates': [],
                'uncertainties': [],
                'offset': None,
            })
            meta['levels'].append(lvl)
            meta['dates'].append(dt.isoformat())
            meta['offset'] = offset  # last BL for that year wins; OK for plotting / info

        # Fill metadata for all years present in df
        years_in_df = sorted(df['datetime'].dt.year.unique())
        for y in years_in_df:
            if y in used_years:
                # pull uncertainties from params if present
                base_uncs = self.get_base_level_uncertainties(station_key, y)
                meta = used_years[y]
                # if a single uncertainty is given, replicate; otherwise just attach list
                if base_uncs:
                    if len(base_uncs) == 1 and len(meta['levels']) > 1:
                        meta['uncertainties'] = [base_uncs[0]] * len(meta['levels'])
                    else:
                        meta['uncertainties'] = base_uncs
                else:
                    meta.setdefault('uncertainties', [])
                self.base_level_metadata[(station_key, y)] = meta
            else:
                # no BL used in this year
                self.base_level_metadata[(station_key, y)] = {
                    'levels': [],
                    'dates': [],
                    'uncertainties': [],
    ...
    def process_year(self, station_key, year, subselection=None):
        info = self.get_station_info(station_key)
        ...
        water_file = self.select_files(
            water_files, year_str, "water", search_string=search_string
        )
        atmo_file = self.select_files(
        ...
        water_full_path = self.main_data_path / info["wl_folder"] / water_file
        atmo_full_path = self.main_data_path / info["atmo_folder"] / atmo_file

        df_wl = self.read_with_timezone(water_full_path)
        df_atmo = self.read_with_timezone(atmo_full_path)
